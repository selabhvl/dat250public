...
2. Run `gradle init` in that location and pick the followinng settings:
```
...
Project name: ...      # choose someting meaningful, e.g. 'dat250exp1'
Source package: ...     # choose soemthing meaningful, e.g. 'no.hvl.dat250.<studid>.exp1'
```
...
The remaining files should be ignored via adding respective entries to your `.gitignore` (if they are not already present).
...
The latter is a library that easily facilitates the development of (REST) web application in a simple manner.
...
Can you find out how many metres 30 feet are?
...
Still, you might agree that the code quality has some room for improvements :wink:.
The whole application is contained in one single `main` method.
This macks the code less clear and also impossible to test.
...
Today, containers are emerging as the de-factor standard for deploying applications both locally and in the cloud.
...
A layer is build via executing a single Linux command. 
...
podman images
```
Can you find your application? Are ther more images other than the application you just build?
...
Make sure to include its URL into the final report.
